...
class ChinaMap extends PureComponent {
  ...
  componentDidMount() {
    // const { id } = this.props;
    // const domNode = document.getElementById(`map-${id}`);
  ...
  initData = (props) => {
    const {
      id, title, data: propData, loading
    } = props;
    const domNode = document.getElementById(`map-${id}`);
    ...
    fetch(`${ROOT_PATH}/data/asset/geo/USA.json`).then((res) => {
      if (res.status >= 200 && res.status < 300) {
        this.myChart.hideLoading();

        console.log('aaaa', res);
        console.log('bbbbb', res?.json());
        const { body: usaJson } = res;
